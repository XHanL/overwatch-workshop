...
function initModelDarkCompletion(context) {
    ...
    //RULES.EVENT["暗色补全"] = []
    for (const i in RULES.EVENT) {
        let item = new vscode.CompletionItem(i)
        item.kind = vscode.CompletionItemKind.Event
        item.documentation = getItemDocumentation(context, i)
        item.filterText = (i + getPinyin(i)).split("").join(" ")
        RULES.EVENT["暗色补全"].push(item)
    }

    //RULES.EVENT_TEAM["暗色补全"] = []
    for (const i in RULES.EVENT_TEAM) {
        let item = new vscode.CompletionItem(i)
        item.kind = vscode.CompletionItemKind.Event
        item.documentation = getItemDocumentation(context, i)
        item.filterText = (i + getPinyin(i)).split("").join(" ")
        RULES.EVENT_TEAM["暗色补全"].push(item)
    }

    //RULES.EVENT_PLAYER["暗色补全"] = []
    for (const i in RULES.EVENT_PLAYER) {
        let item = new vscode.CompletionItem(i)
        item.kind = vscode.CompletionItemKind.Event
        item.documentation = getItemDocumentation(context, i)
        item.filterText = (i + getPinyin(i)).split("").join(" ")
        if (RULES.EVENT_PLAYER[i].hasOwnProperty("顺序")) {
            item.sortText = RULES.EVENT_PLAYER[i].顺序
        }
        RULES.EVENT_PLAYER["暗色补全"].push(item)
    }
}
...
function initModelLightCompletion(context) {
    for (i in CONSTS) {
        ...
        for (j in CONSTS[i]) {
            CONSTS[i][j]["亮色补全"].push(new vscode.CompletionItem(j))
        }
    }
    for (i in RULES) {
        for (j in RULES[i]) {
            RULES[i][j]["亮色补全"].push(new vscode.CompletionItem(j))
        }
    }
}
...
function getItemDocumentation(context, name) {
    const documentation = new vscode.MarkdownString(`**${name}**`)
    documentation.isTrusted = true
    documentation.supportHtml = true
    documentation.supportThemeIcons = true
    documentation.baseUri = vscode.Uri.file(path.join(context.extensionPath, '', path.sep))
