...
function activate(context) {
    ...
    context.subscriptions.push(
        ...
        //取色
        vscode.languages.registerColorProvider("json", {
            provideDocumentColors(document) {
                let colors = []
                const pattern = /#([0-9a-fA-F]{6}([0-9a-fA-F]{2})?|[0-9a-fA-F]{3}([0-9a-fA-F]{1})?)/
                for (let i = 0; i < document.lineCount; i++) {
                    const line = document.lineAt(i);
                    const match = pattern.exec(line.text)
                    if (match) {
                        const red = parseInt(match[0].slice(1, 3), 16) / 255
                        const green = parseInt(match[0].slice(3, 5), 16) / 255
                        const blue = parseInt(match[0].slice(5, 7), 16) / 255
                        let alpha = 1.0
                        if (match[0].length === 9) {
                            alpha = parseInt(match[0].slice(7, 9), 16) / 255
                        }
                        colors.push(
            ...
            provideColorPresentations(color) {
                if (Math.floor(color.alpha) == 1) {
                    return [new vscode.ColorPresentation('#' +
                        Math.floor(color.red * 255).toString(16).padStart(2, '0') +
                        Math.floor(color.green * 255).toString(16).padStart(2, '0') +
                        Math.floor(color.blue * 255).toString(16).padStart(2, '0'))]
                } else {
                    return [new vscode.ColorPresentation('#' +
                        Math.floor(color.red * 255).toString(16).padStart(2, '0') +
                        Math.floor(color.green * 255).toString(16).padStart(2, '0') +
                        Math.floor(color.blue * 255).toString(16).padStart(2, '0') +
                        Math.floor(color.alpha * 255).toString(16).padStart(2, '0'))]
                }
            }
        }),
        vscode.languages.registerColorProvider("ow", {
            provideDocumentColors(document) {
                let colors = []
                const pattern = /自定义颜色\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/
                for (let i = 0; i < document.lineCount; i++) {
                    const line = document.lineAt(i);
                    const match = pattern.exec(line.text)
                    if (match) {
