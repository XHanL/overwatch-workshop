...
- 打开任意 `.ow` 文件
- 从工坊复制 `完整代码` 到剪切板
- 点击右上方的 `导入` 按钮，或右键菜单中的 `导入` 选项
- 插件将自动修复剪切板中的工坊错误，然后覆盖到 `.ow` 文件中

### ❺ 导出代码

- 打开任意 `.ow` 文件
- 点击右上方的 `导出` 按钮，或右键菜单中的 `导出` 选项
- 插件将自动修复 `.ow` 文件中的工坊错误，然后覆盖到剪切板中

### ❻ 建议列表

- 点击文本将光标定位到目标位置
- 点击右上方的 `建议` 按钮，或右键菜单中的 `建议` 选项
- 插件将识别上下文并给出可能的选项列表
- 使用方向键 `上` 和 `下` 滚动列表
